// ### prevNext helper exposes methods for prev_post and next_post - separately
// defined in helpers index.
//  Example usages
// `{{#prev_post}}<a href ="{{url}}>previous post</a>{{/prev_post}}'
// `{{#next_post}}<a href ="{{url absolute="true">next post</a>{{/next_post}}'

var api = require('../api'), schema = require('../data/schema').checks,
    Promise = require('bluebird'), fetch, prevNext;

fetch = function(apiOptions, options) {
  return api.posts.read(apiOptions)
...
// If prevNext method is called without valid post data then we must return a
// promise, if there is valid post data
...
prevNext = function(options) {
  ...
  var apiOptions = {
    include : options.name === 'prev_post'
